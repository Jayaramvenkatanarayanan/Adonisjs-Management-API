...
class EmployeeController {
    ...
    async Store({request, response}) {
        ...
        const empSalInput = request.only(['salary','emp_no', 'from_date', 'to_date']);
        const empTitleInput = request.only(['title']);
        ...
        const empTitleValidation = await Validator.validate(empTitleInput, Emptitle.AddEmpTitle, Emptitle.AddEmpTitleError);
        ...
        if (empTitleValidation.fails()) {
            return response.status(400).json({
                data: empTitleValidation._errorMessages[0].message,
                message: 'Employee registration fail',
                status: false
            });
        }
        ...
        var empTitle = {
            'emp_no': empInput.emp_no, 'title': empTitleInput.title,
            'from_date': empSalInput.from_date,
            'to_date': empSalInput.to_date
        };
        ...
        await dataBase.transaction(async (trx, err) => {
            ...
            await trx.insert(empTitle).into('emp_titles');
